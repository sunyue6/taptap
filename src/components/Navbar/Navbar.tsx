...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`z-50 fixed top-[16px] left-0 w-full text-textPrimary ${hanalei.className} px-[16px]`}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between w-full h-[60px] mmd:h-[74px]">
          <div className="flex items-center cursor-pointer">
            <Image
              ...
              width={44}
              height={37.29}
              className="mr-[12px] mt-[15px]"
            />
            <div className="text-textPrimary hidden mmd:block text-[30px]">
              TAPTAP
            </div>
          </div>
          <div className="flex">
        ...
        <div
          className={`w-full mmd:hidden ${hanalei.className}  ${menuOpen ? 'block' : 'hidden'} text-textPrimary mt-[32px] pb-[48px]`}
        >
          <div>
            {navItems.map((item, index) => (
              ...
              >
                <div className="text-[24px]">{item.label}</div>
